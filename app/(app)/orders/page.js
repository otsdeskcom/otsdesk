...
async function update(user, orderId, patch) {
  ...
  const DATE_FIELDS = new Set(['order_date','ship_by','est_delivery','exp_delivery','supplier_est_delivery','label_created_date']);
  const vals = fields.map(f => {
    let v = patch[f];
    if (v === '' || v === undefined) return null;                 // empty → NULL (safe for dates/numbers)
    if (NUMERIC.has(f)) { const n = Number(v); return isNaN(n) ? null : n; }
    if (DATE_FIELDS.has(f)) return String(v).slice(0, 10) || null; // keep YYYY-MM-DD
    return v;
  });
  const o = await db.one(
